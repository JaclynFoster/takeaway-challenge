...
// Then, you can absolutely determine who would win a game with n stones.
// For example:
// 1
...
// Remember: both players make the best possible moves, so if player #1 has a winning strategy, 
//she will use it (and player #2 will defend as well as possible).
// Code
// We’ve written a function which takes one parameter, n, the number of stones in the game.
// It should return true if the active play could win a game starting with that number of stones 
//and false if not. (Hint: Use recursion)
// For example:
// canWin(1)
// // false
// canWin(2)
...
// // true
const canWin = (n) => {
    //none of the moves are under 2
if (n < 2) {
    return false
}
//if can win is false for the opponent then we win
...
//if we can't win above then we lose
    return false
}
